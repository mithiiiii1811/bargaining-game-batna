...
const waitingQueues = { 1: [], 2: [], 3: [] };
const socketsGroup = new Map();
const socketsRoom = new Map();
const socketsRole = new Map();
const sessions = {};
const app = express();
app.use(cors());
...
function pairPlayers(group) {
    ...
    if (q.length >= 2) {
        ...
        const roles = Math.random() < 0.5 ? ["seller", "buyer"] : ["buyer", "seller"];
...
io.on("connection", (socket) => {
    ...
    socket.on("offer", (payload) => {
        ...
        if (!sess)
            return;
        const evt = { t: Date.now(), actor: role, price: payload.price };
